...
func IsWrapped(err error) bool {
	return failure.Is(err, ErrTypeUnAuthorized, ErrTypeUnAuthorization, ErrTypeBussiness, ErrTypeCritical, ErrTypeNotFound, ErrTypeConflict)
}
...
func GetCode(err error) ErrCode {
	...
	if code, ok := types.(ErrType); ok {
		...
		case ErrTypeUnAuthorization:
			return ErrCodeUnAuthorization
